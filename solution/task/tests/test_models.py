...
class TaskModelTestCase(TestCase):
    ...
    def test_create_valid_task_and_defaults(self):
        ...
        self.assertEqual(t.tags, [], "tags default should be an empty list")
    ...
    def test_tags_accepts_dict_and_lists(self):
        # tags can be any JSON-serializable value
        d = {"k": "v"}
        arr = ["a", 1, {"b": 2}]
        t1 = Task.objects.create(title="dict-tags", created_by=self.user, tags=d)
        t2 = Task.objects.create(title="list-tags", created_by=self.user, tags=arr)
        self.assertEqual(t1.tags, d)
        self.assertEqual(t2.tags, arr)

        # non json serializable values
        t = Task(title="bad-tags", created_by=self.user)
        t.tags = set([1, 2]) 
    ...
    def test_created_by_fk_constraint_and_default_0_behavior(self):
        ...
        # maybe creating an user and then deleting it to get a non-existent id would work
        # but it throws value error not integrity error
        # self.user.delete()
        # t2 = Task(title="bad-user-id", created_by=self.user)
        # with self.assertRaises(IntegrityError):
        #     with transaction.atomic():
        #         t2.save()
